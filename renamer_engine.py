"""
Core renaming engine for the Bulk File Renamer GUI application.
This module contains the backend logic for file renaming operations.
"""
...
class BulkRenameConfig:
    ...
    def validate(self):
        ...
        if bool(self.regex_pattern) != bool(self.regex_replacement):
            if self.regex_pattern or self.regex_replacement:
                raise ValueError(
                    "Both regex pattern and replacement are required together."
                )


def preview_renames(config: BulkRenameConfig):
    """
    Generate a preview of rename operations without actually renaming files.
    Returns a list of dictionaries with 'original_name' and 'new_name' keys.
    """
    config.validate()
...
def rename_files(config: BulkRenameConfig):
    ...
    Returns a dictionary with rename results.
    """
    config.validate()
    files = _collect_files(config.directory, config.extensions)
...
def _build_new_name(original_name, config, counter):
    ...
    # Apply regex transformation first
